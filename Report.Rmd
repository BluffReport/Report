...
ggplot(data, aes(x = orientation, y = max.wave.height, color = as.factor(seawall), alpha = 1 - mud.comp, size = rr)) +
  geom_point() +  
  geom_vline(xintercept = 15, linetype = "dashed", color = "darkblue", size = 1) +  
  scale_color_manual(values = c("#FF6666", "#66B2FF"), name = "Seawall Presence") +
  scale_alpha_continuous(name = "Mud Content", range = c(0.1, 1)) +
  scale_size_continuous(name = "Retreat Rate (RR)", range = c(1, 4)) +
  labs(x = "Degree Orientation (°)",
       title = "Orientation vs. Max Wave Height",
) +
