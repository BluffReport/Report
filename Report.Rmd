...
From these visualizations, it can be deduced that fitting a regression model to the data may be unreliable. The assumption of independent bluff observations appears to be violated. Many bluffs in close proximity to one another on one island suggests that the variables measured may be correlated and not independent. Fitting a regression to determine the significance of certain variables on retreat rate would then produce unreliable and inaccurate results. The appendix further shows each variable's random relationship with retreat rate.
