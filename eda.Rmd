---
title: "linear regression model and factor analysis for erosion"
...
```{r}
model4 <- lm(`RR..m.yr.` ~ `BluffEl..m.`, data = data)
summary(model4)
...
```{r}
model5 <- lm(`RR..m.yr.` ~ `Orientation..deg.`, data = data)
summary(model5)
...
```{r}
model6 <- lm(`RR..m.yr.` ~ `BaseEl..m.`, data = data)
summary(model)
